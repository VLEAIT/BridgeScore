...
class Application(Base, TimeStampMixin, kw_only=True):
    ...
    land_area_hectares: Mapped[float] = mapped_column(Float,nullable=False, comment="Land area in hectares")
    ...
    coop_income_monthly: Mapped[Decimal] = mapped_column(Numeric(precision=12,scale=2), nullable=False, comment="Monthly income of the farmer from cooperative farming")
    remittance_monthly: Mapped[Optional[Decimal]] = mapped_column(Numeric(precision=12,scale=2), nullable=True, default=None, comment="Monthly remittance received by the farmer")
    ...
    requested_amount: Mapped[Decimal] = mapped_column(Numeric(precision=12,scale=2), nullable=False, comment="Amount requested by the user in nrs")
